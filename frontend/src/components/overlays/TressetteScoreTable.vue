<template>
  ...
  <div v-if="showDetail" class="score-detail-backdrop" @click.self="showDetail = false">
    <div class="score-detail-dialog" role="dialog" :aria-label="t('tressette.score.cards')">
      <button class="score-detail-close" :aria-label="t('dialog.close')" @click="showDetail = false">&times;</button>
      <h3>{{ t('tressette.score.cards') }}</h3>
...
function countCardPoints(cards: Card[]): number {
  return cards.reduce((sum, c) => sum + (TRESSETTE_CARD_POINTS[c.value] ?? 0), 0)
}

const myCardPoints = computed(() => countCardPoints(myCards.value))
const oppCardPoints = computed(() => countCardPoints(oppCards.value))

// Ultima bonus: total - card points (server already added it)
const myUltima = computed(() => props.myTotalScore - myCardPoints.value)
const oppUltima = computed(() => props.oppTotalScore - oppCardPoints.value)

/** Sort cards by Tressette point value descending. Exclude 0-point cards. */
function sortByPoints(cards: Card[]): Card[] {
  return [...cards]
    .filter((c) => (TRESSETTE_CARD_POINTS[c.value] ?? 0) > 0)
    .sort((a, b) => {
      const pa = TRESSETTE_CARD_POINTS[a.value] ?? 0
      const pb = TRESSETTE_CARD_POINTS[b.value] ?? 0
